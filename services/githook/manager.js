...
class Manager {
    ...
    async change(auth, params, body){      
        ...
        const gitEvent = await this._decode(body);  

        this.persistentStore.addEvent(gitEvent);

        await this._emitStats();
        
        log.log(`Processed ${gitEvent.remote} with score: ${gitEvent.s} and message: ${gitEvent.decoded.message}`)    
    }
     ...
     * @deprecated switched to ShanonEntropyScoreModel
     */
    _score(item){
