    ...
    async showData2(message){
        ...
        message.users.forEach((user)=>{
            ...
            user.projects.forEach((project)=>{
                ...
                project.hide = Object.keys(that.model.messages).length>0?that.model.messages.users.find((item)=>{return item.id == user.id}).projects.find((item)=>{return item.id == project.id}).hide:false
                project.tasks.forEach((task)=>{
